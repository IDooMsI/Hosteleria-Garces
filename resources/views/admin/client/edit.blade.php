        ...
        <div class="form-row">
            ...
            <div class="form-row mr-1 col-12 col-md-6 col-lg-6">
                ...
                <div class="form-group m-0 col-12 col-md-6 col-lg-6">
                    ...
                    <input name="street" class="form-control" type="text" value="{{ $client->address->street }}">
                    @error('street')
                        <div id="error" class="alert alert-danger mx-auto col-11 col-sm-4 col-lg-12"><span>{{ $message }}</span></div>
                    @enderror
                </div>
                <div class="form-group col-12 col-md-6 col-lg-6">
                    ...
                    <input name="number" class="form-control" type="text" value="{{ $client->address->number }}">
                    @error('number')
                        <div id="error" class="alert alert-danger mx-auto col-11 col-sm-4 col-lg-12"><span>{{ $message }}</span></div>
                    @enderror
                </div>
                    ...
                    <select class="form-control mb-1" name="locality">
                        ...
                        @if(isset($localities))
                            @foreach($localities as $locality)
                                <option <?php if ($locality->id == $client->address->locality->id) echo "selected"; ?> value="{{$locality->id}}">{{Ucfirst($locality->name)}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
                    ...
                    <select class="form-control col-12" name="provider">
                        ...
                        @if(isset($providers))
                            @foreach($providers as $provider)
                                <option <?php if ($provider->id == $client->provider_id) echo "selected"; ?> value="{{$provider->id}}">{{Ucfirst($provider->name)}}</option>
                            @endforeach
                        @endif
                    </select>
                </div>
